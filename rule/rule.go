...
type ProbRule [NumStates][NumStates][NumStates]Probability
...
func LoadRule(fn string) Rule {
	...
	for scanner.Scan() {
		var ln, c, rn, state string
		// var s1, s2, s3, s4 string
		// var p1, p2, p3, p4 float64

		fmt.Sscanf(r.Replace(scanner.Text()), "%s %s %s %s", &ln, &c, &rn, &state)
		// fmt.Println(rules.String2State(ln), rules.String2State(c), rules.String2State(rn), p1, p2, p3, p4)
		ru[String2State(ln)][String2State(c)][String2State(rn)] = String2State(state)
	}
	return ru
}

func LoadProbRule(fn string) ProbRule {
	var pr ProbRule
	f, err := os.Open(fn)
	if err != nil {
		fmt.Println("ERROR: reading rule", err)
	...
	for scanner.Scan() {
		...
		// fmt.Println(rules.String2State(ln), rules.String2State(c), rules.String2State(rn), p1, p2, p3, p4)
		pr[String2State(ln)][String2State(c)][String2State(rn)][0] = p1
		pr[String2State(ln)][String2State(c)][String2State(rn)][1] = p2
		pr[String2State(ln)][String2State(c)][String2State(rn)][2] = p3
		pr[String2State(ln)][String2State(c)][String2State(rn)][3] = p4
	}
	return pr
}
